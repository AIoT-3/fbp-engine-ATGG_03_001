...
class ReadHoldingRequestPduTest {

    @Test
    @DisplayName("생성된 PDU를 인코딩하면, Modbus PDU 스펙와 일치한다.")
    void encode_WhenCreateAndEncode_CorrectsModbusPdu() {
        // given
        ReadHoldingRequestPdu pdu = new ReadHoldingRequestPdu(0, 1);

        // when
        byte[] actual = pdu.encode();

        // then
        // Function Code(1), Start Address(2), Quantity(2)
        byte[] expected = { 0x03, 0x00, 0x00, 0x00, 0x01 };
    ...
    @Test
    @DisplayName("Modbus PDU 바이트 배열을 읽으면, ReadHoldingRequestPdu 객체로 매핑할 수 있다.")
    void read_WhenPduByteArray_ReturnPduEntity() throws IOException  {
        // given
        byte[] given = { 0x03, 0x00, 0x00, 0x00, 0x01 };
        ...
        // when
        ReadHoldingRequestPdu pdu = ReadHoldingRequestPdu.read(in);
        ...
        assertSoftly(softly -> {
            ...
            softly.assertThat(pdu.startAddress())
                    .isEqualTo(buffer.getShort(1));
            softly.assertThat(pdu.quantity())
